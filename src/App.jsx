...
// === ระบบบีบอัดรูปภาพเป็น Blob สำหรับอัปโหลดขึ้น Storage (ชัดขึ้นมาก) ===
export const compressImageToBlob = (file, callback) => {
  ...
  reader.onload = (event) => {
    ...
    img.onload = () => {
      ...
      const MAX_WIDTH = 1200; // ปรับเพิ่มเป็น 1200px ให้ภาพชัดระดับ HD
      let width = img.width;
      ...
      // คืนค่าเป็น Blob (ไฟล์) และ Quality 80% เพื่อเตรียมโยนขึ้น Storage
      canvas.toBlob((blob) => {
        callback(blob);
      }, 'image/jpeg', 0.8); 
    };
...
// ปรับปรุงคอมโพเนนต์อัปโหลดรูป ให้มีแอนิเมชันตอนอัปโหลด
const ImageUpload = ({ label, onChange, value, height = "h-24", onZoom }) => {
  ...
  const handleFile = (e) => {
    if (e.target.files && e.target.files[0]) {
      setIsUploading(true);
      compressImageToBlob(e.target.files[0], async (blob) => {
        try {
          const fileRef = ref(storage, `trials_images/${Date.now()}_${Math.random().toString(36).substring(2,7)}.jpg`);
  ...
  return (
    <div className={`border-2 border-dashed ${isUploading ? 'border-blue-300 bg-blue-50' : 'border-gray-300'} rounded-lg p-2 text-center hover:bg-gray-50 relative ${height} flex flex-col items-center justify-center bg-white group overflow-hidden`}>
      <input type="file" accept="image/*" className="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10 disabled:cursor-not-allowed" onChange={handleFile} disabled={isUploading} title={value ? "คลิกเพื่อเปลี่ยนรูป" : "คลิกเพื่อเพิ่มรูป"} />
      
      {isUploading ? (
         <div className="flex flex-col items-center justify-center z-20">
            <div className="w-5 h-5 border-2 border-blue-500 border-t-transparent rounded-full animate-spin mb-1"></div>
            <span className="text-[9px] text-blue-600 font-bold">อัปโหลด...</span>
         </div>
      ) : value ? (
        <>
...
export default function App() {
  ...
  const handleExportExcel = (elementId, filename) => {
    ...
    try {
      ...
      alert('ส่งออก Excel สำเร็จ! \n*หมายเหตุ: รูปภาพที่อัปโหลดไปแล้วในฐานข้อมูลจะไม่ถูกส่งออกใน Excel ครับ');
    } catch (error) {
                             ...
                             <button type="button" onClick={() => {
                                ...
                                input.onchange = (e) => {
                                    if(e.target.files && e.target.files[0]) {
                                        ...
                                        compressImageToBlob(e.target.files[0], async (blob) => {
                                            try {
                                                const fileRef = ref(storage, `proofs/${Date.now()}_${Math.random().toString(36).substring(2,7)}.jpg`);
                                                await uploadBytes(fileRef, blob);
                                                const downloadUrl = await getDownloadURL(fileRef);
                                                setBookingData(prev => ({...prev, proofImages: [...(prev.proofImages || []), {id: Date.now(), img: downloadUrl}]}));
                                            } catch(err) {
      ...
      return (
        <div className="bg-blue-50 p-6 rounded-xl shadow-lg border-2 border-blue-200 col-span-1 md:col-span-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className={`flex flex-col items-center justify-center border-2 border-dashed ${isUploadingPartImg ? 'border-blue-300 bg-blue-50' : 'border-gray-300'} rounded-lg p-2 bg-white relative min-h-[160px]`}>
               <input type="file" accept="image/*" disabled={isUploadingPartImg} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10 disabled:cursor-not-allowed" onChange={(e) => {
                  if (e.target.files && e.target.files[0]) {
                     setIsUploadingPartImg(true);
                     compressImageToBlob(e.target.files[0], async (blob) => {
                        try {
                           const fileRef = ref(storage, `parts_images/${Date.now()}_${Math.random().toString(36).substring(2,7)}.jpg`);
                           await uploadBytes(fileRef, blob);
                           const downloadUrl = await getDownloadURL(fileRef);
                           setPartInput({...partInput, img: downloadUrl});
                        } catch(err) {
                           alert('อัปโหลดรูปไม่สำเร็จ');
                        } finally {
                           setIsUploadingPartImg(false);
                        }
                     });
                  }
               }} />
               {isUploadingPartImg ? (
                  <div className="text-center text-blue-600 flex flex-col items-center z-20">
  ...
  const TrialForm = () => {
    ...
    const handleSave = async (statusType) => {
      ...
      try {
        ...
        alert("ไม่สามารถบันทึกข้อมูลได้!\n\nสาเหตุที่เป็นไปได้: ขนาดของข้อมูลและรูปภาพรวมกันใหญ่เกินไป\nกรุณาลดจำนวนรูปลง หรือลองอีกครั้งครับ");
      } finally {
        setIsSaving(false);
      }
    };
